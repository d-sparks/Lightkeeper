...
## 2. [sonnet] Fix relay_recovery quest soft-lock

The `junction_a_activated` flag never gets set, so the `find_relay` step never completes. The quest expects the player to interact with a junction box in relay_station that sets this flag — either the trigger is missing, the interactable tile doesn't fire the right action, or the flag name is mismatched. Trace from the relay_station dungeon triggers and tileset interactions to find the break. This blocks one of 14 quests from completing.

## 3. [sonnet] Fix broken_signal quest timeout

The bot reaches signal_cave but the `signal_source_found` flag never gets set, causing a 40-minute timeout. Check signal_cave.json triggers — the flag should set on room entry or on interacting with the dead Lightkeeper's camp. Likely a missing `room_entered` trigger or an interaction the bot can't reach.

## 4. [sonnet] Fix explore-mode workshop flag gate
...
## 5. [opus] Difficulty tuning pass — 0 deaths is too easy
...
## 6. [sonnet] Connect crypt dungeons to the narrative
...
## 7. [sonnet] Wire lighthouse_siege_arena to player-facing trigger
...
## 9. [opus] Add environmental storytelling to the Nightside path
...
## 11. [sonnet] Level-up stat screen
...
## 12. [sonnet] Weapon upgrade confirmation dialog
...
## 14. [opus] Add Dural Voss as a named boss encounter
...
## 15. [opus] Build Spire of Vigil — outer layer (raider stronghold)
...
## 16. [opus] Build Greenway corridor dungeons + Spire of Winds
...
## 18. [sonnet] Replace top-priority placeholder sprites
...
## 19. [opus] Deep Expedition (Tier 6) cooperative content
...
## 20. [sonnet] nightside_expedition quest — fix return navigation timeout
