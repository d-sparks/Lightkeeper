...
- ~~**Quarantine sim loop (CRITICAL — blocks mainline)**~~ — FIXED: Added depth recovery retry limit (5 attempts) to `wait_for_item`, once-trigger flag clearing on retry/reset (so `supply_crate_key` can re-spawn), and moved `trackCombatEvents` before `processTransitions` to fix death tracking (deaths were lost when rooms destroyed on respawn). Bot now fails gracefully with TIMEOUT after 6 depth recovery attempts instead of looping indefinitely. **Remaining**: Bot still can't survive quarantine depth 3 (7 deaths in ~400s) — fights at depths 1-2 deplete health before reaching the warlord. Consider: reduce combat engagement at transit depths, add health-gate logic before entering boss depth, or tune quarantine monster budget downward.
