...
- ~~Path-specific structures currently appear unlocked in automation screens opened outside the AUTO_BUILD flow (join/room-enter auto state send, scripting openAutomation). Pass pathFlags to all getStateForClient call sites for consistent locked display.~~ ✓ Fixed — `Automation` now holds a `flagStore` reference (injected in game-loop.js); `_getPathFlags(playerId)` builds the flag map on demand; `getStateForClient` resolves flags automatically when the caller omits them.
...
- ~~Siege lighthouse repair interaction — design doc mentions spending silicon to repair between waves.~~ ✓ Implemented — SIEGE_REPAIR message type, trySiegeRepair() in game-loop.js spends silicon (automation resource) and restores lighthouse HP. Repair button appears in siege HUD during inter-wave/preparing phases when lighthouse is damaged. Cost/amount from challenge config (lighthouseRepairCost/lighthouseRepairAmount). Visual feedback: green +HP number at lighthouse, item_pickup SFX.
...
- ~~Siege visual feedback — lighthouse entity rendering, wave start/clear announcements, victory/defeat overlay.~~ ✓ Implemented — procedural lighthouse rendering with HP-based color, pulsing beacon, damage numbers, wave announcements, victory/defeat overlays.
...
- ~~Wire expedition_tier_4_cleared and expedition_tier_5_cleared flags.~~ ✓ Implemented — `expedition_board` dialogues/rules extended for tiers 3–5 (each state shows all available tiers up to next unlock; tier 5 shows apex cleared message); Sable Shutdown/Merge paths now have `shutdown/merge_tier_4/5_complete` dialogue sets reflecting the Voidfront and Apex Breach depths; all rules ordered highest-tier-first.
